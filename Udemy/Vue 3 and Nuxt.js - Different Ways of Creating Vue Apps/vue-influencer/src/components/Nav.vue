<template>
  <div class="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 border-bottom shadow-sm">
    ...
    <nav class="my-2 my-md-0 mr-md-3" v-if="isAuthenticated">
    ...
    <router-link to="/login" class="btn btn-outline-primary" v-if="!isAuthenticated">Login</router-link>
    <router-link to="/profile" class="btn btn-outline-primary" v-else>{{user?.first_name}} {{user?.last_name}}</router-link>
