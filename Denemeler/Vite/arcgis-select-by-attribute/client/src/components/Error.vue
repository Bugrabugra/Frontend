<template>
  <div class="fixed overflow-x-hidden overflow-y-auto inset-0 flex justify-center items-center z-50">
    <div @click="clickOutside" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full"></div>
    <div
    ...
    >
      <div class="bg-white w-full px-2 py-2 rounded-lg border-t-[10px] border-teal-300 h-24 shadow-2xl flex flex-col items-center justify-center">
        ...
        <button class="button" @click="closeError">
          Close
        </button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      return {closeError, clickOutside, outside}
    }
  }
</script>