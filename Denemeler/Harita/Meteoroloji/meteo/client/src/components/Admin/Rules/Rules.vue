    ...
    <div class="px-4 py-2">
      ...
      <div class="mt-2 border border-gray-400 rounded-xl px-2 py-2 grid grid-cols-12 gap-y-2
        ...
        <div class="col-span-12 md:col-span-10 flex flex-col flex-nowrap space-y-1">
          ...
          <div class="text-xs text-red-600" v-if="errorMinValue">
            *{{errorMinValue["message"]}}
          </div>
          ...
          <p class="text-xs text-red-600" v-if="errorMaxValue">
            *{{errorMaxValue["message"]}}
          </p>

          <!--warning message-->
          <div class="flex">
            <label class="w-1/3 py-1 font-bold text-gray-700">Uyarı mesajı:</label>
            <textarea rows="4" class="w-2/3 users-input" v-model="inputWarningMessage"/>
          </div>
          <p class="text-xs text-red-600" v-if="errorWarningMessage">
            *{{errorWarningMessage["message"]}}
          </p>

          <!--groups-->
          <div class="flex">
            <label class="w-1/3 py-1 font-bold text-gray-700">Gruplar:</label>
            <input class="w-2/3 users-input" type="text" v-model="inputGroups">
          </div>
          <p class="text-xs text-red-600" v-if="errorGroups">
            *{{errorGroups["message"]}}
          </p>
        </div>
...
<script setup>
  ...
  const selectedRule = computed(() => {
    return store.state.rules.ruleSelected;
  });
  ...
  const vuelidateRules = computed(() => {
    return {
      ...
      inputMinValue: {
        required: helpers.withMessage(
            "Minimum değer girilmelidir",
            required
        )
      },
      inputMaxValue: {
        required: helpers.withMessage(
            "Maksimum değer girilmelidir",
            required
        )
      },
      inputWarningMessage: {
        required: helpers.withMessage(
            "Uyarı mesajı girilmelidir",
            required
        )
      },
      inputGroups: {
        required: helpers.withMessage(
            "Kullanıcı grubu girilmelidir",
            required
        )
      }
    }
  ...
  const errorMinValue = computed(() => {
    return vuelidateErrors.value.filter(error => {
      if (error.property === 'inputMinValue') {
        return error.message;
      }
    })[0];
  });

  const errorMaxValue = computed(() => {
    return vuelidateErrors.value.filter(error => {
      if (error.property === 'inputMaxValue') {
        return error.message;
      }
    })[0];
  });

  const errorWarningMessage = computed(() => {
    return vuelidateErrors.value.filter(error => {
      if (error.property === 'inputWarningMessage') {
        return error.message;
      }
    })[0];
  });
  ...
  const createRule = async () => {
    ...
    if (!v$.value.$error) {
      const rule = {
        ...
        warning_message: inputWarningMessage.value,
        groups: inputGroups.value
      };
  ...
  watch(selectedRule, newValue => {
    ...
    inputWarningMessage.value = newValue.warning_message;
    inputGroups.value = newValue.groups;
  })
</script>