...
function mapEventObject(event) {
  ...
  if (startDate.date !== endDate.date) {
    dateRange = `${startDate.month} ${startDate.date}-${startDate.month} ${startDate.date}`;
...
async function loadEvents(max = 8) {
  try {
    ...
    const processedData = data.map((e) => mapEventObject(e));
    eventContainer.innerHTML = processedData.map((event, i) => createEvent(event, i)).join("");
